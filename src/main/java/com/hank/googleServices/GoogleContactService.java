package com.hank.googleServices;
...
public class GoogleContactService {
    ...
    private static Credential getCredentials(final NetHttpTransport HTTP_TRANSPORT) throws IOException {
        // Load client secrets.
        InputStream in = GoogleContactService.class.getResourceAsStream(CREDENTIALS_FILE_PATH);
        if (in == null) {
    ...
    public static void main(String... args) throws IOException, GeneralSecurityException {
        ...
        if (connections != null && connections.size() > 0) {
            System.out.println("Total Contacts: " + connections.size());
            for (Person person : connections) {
                List<Name> names = person.getNames();
                if (names != null && names.size() > 0) {

                    System.out.println("Name: " + person.getNames().get(0)
                            .getDisplayName() + " <--> " + person.getPhoneNumbers());
                } else {
